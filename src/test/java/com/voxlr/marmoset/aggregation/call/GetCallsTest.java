...
public class GetCallsTest extends CallAggregationBaseTest {
  ...
  public void getCallsByCompanyReturnsEmptyIfNoCallsExist() throws Exception {
    Page<CallAggregateDTO> results =
        callAggregation.getCallsByCompany(
            mockCompany.getId(),
            new DateTime(),
            new DateTime(),
  ...
  public void getCallsByCompanyShouldReturnValidListWithCorrectData() throws Exception {
    ...
    assertThat(
        result.getDetectedPhraseCount(), is(expected.getAnalysis().getDetectedPhraseCount()));
  ...
  public void getCallsByCompanyShouldReturnMultipleCallsWithinDateRange() {
    ...
    assertThat(
        results.getContent(), containsMatch(result -> result.getId().equals(withinRange.getId())));
    assertThat(
        results.getContent(), containsMatch(result -> result.getId().equals(withinRange2.getId())));
  }
  ...
  public void getCallsByUserReturnsEmptyIfNoCallsExist() throws Exception {
    Page<CallAggregateDTO> results =
        callAggregation.getCallsByUser(
            mockUser.getId(),
            new DateTime(),
            new DateTime(),
            CallAggregationFields.CallField.getAll(),
