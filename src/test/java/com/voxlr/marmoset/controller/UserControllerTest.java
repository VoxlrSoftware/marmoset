...
public class UserControllerTest extends ControllerTest {
  ...
  public void getShouldReturnExceptionIfEntityDoesNotExist() throws Exception {
            ...
            new Answer<User>() {
              ...
              public User answer(InvocationOnMock invocation) throws Throwable {
                throw new EntityNotFoundException(
                    User.class, "id", (String) invocation.getArguments()[0]);
              }
            });

    String fakeId = "123";
    RequestBuilder requestBuilder = get("/api/user/" + fakeId).accept(APPLICATION_JSON);
    ...
    assertThat(
        error.getString("message"), is("User was not found for parameters {id=" + fakeId + "}"));
  }

  @Test
  public void getShouldReturnValidUser() throws Exception {
    when(userService.get(eq(mockUser.getId()), any(AuthUser.class))).thenReturn(mockUser);

    RequestBuilder requestBuilder = get("/api/user/" + mockUser.getId()).accept(APPLICATION_JSON);
    MvcResult result = mvc.perform(requestBuilder).andReturn();
  ...
  public void postShouldReturnErrorWhenCompanyDoesNotExist() throws Exception {
    ...
    assertThat(error.getString("message"), is("Validation error"));
    assertThat(
        error.getJsonArray("subErrors").getJsonObject(0).getString("field"), is("companyId"));
  }
  ...
  public void postWithInvalidBodyShouldReturnException() throws Exception {
    RequestBuilder requestBuilder =
        post("/api/user").accept(APPLICATION_JSON).contentType(APPLICATION_JSON);
    MvcResult result = mvc.perform(requestBuilder).andReturn();
  ...
  public void putShouldReturnValidUser() throws Exception {
    when(userService.update(any(UserUpdateDTO.class), any(AuthUser.class))).thenReturn(mockUser);

    String body = createObjectBuilder().add("name", "New Name").build().toString();
    RequestBuilder requestBuilder =
        put("/api/user/" + mockUser.getId())
  ...
  public void putWithInvalidBodyShouldReturnException() throws Exception {
    RequestBuilder requestBuilder =
        put("/api/user/" + mockUser.getId()).accept(APPLICATION_JSON).contentType(APPLICATION_JSON);
    MvcResult result = mvc.perform(requestBuilder).andReturn();
  ...
  public void deleteShouldReturnValidResponse() throws Exception {
    RequestBuilder requestBuilder =
        delete("/api/user/" + mockUser.getId())
            .accept(APPLICATION_JSON)
            .contentType(APPLICATION_JSON);
    MvcResult result = mvc.perform(requestBuilder).andReturn();
  ...
  private MvcResult doPostWithUser() throws Exception {
    String body =
        createObjectBuilder()
            ...
            .add("email", mockUser.getEmail())
            .build()
            .toString();

    RequestBuilder requestBuilder =
        post("/api/user").accept(APPLICATION_JSON).contentType(APPLICATION_JSON).content(body);
