...
public abstract class CallbackHandler<T> {
  public abstract CallbackResult<T> handleRequest(String requestPath, CallbackBody callbackBody)
  ...
  public final void initialize(ApplicationContext applicationContext) {
    try {
      ...
      throw new RuntimeException(
          "Unable to initialize CallbackHandler ["
              + this.getClass()
              + "]. "
              + "Check that all fields that require wiring are marked with @Autowired.",
          e);
    }
  }
}
