...
import static com.voxlr.marmoset.aggregation.field.AggregationField.getFieldProjections;
import static com.voxlr.marmoset.aggregation.field.AggregationField.isGroupable;
import static com.voxlr.marmoset.aggregation.field.AggregationField.isProjectable;
...
public class CallAggregation extends AbstractAggregation<Call> {
	...
	ProjectionOperation projectionOperation = reduce(project(), getFields(fields), AggregationField::projectField);
    ...
    public RollupResultDTO averageCallFields(Criteria matchCriteria, List<CallField> fields) throws InvalidArgumentsException {
	...
	CustomAggregation<Call> aggregation = anAggregation()
		.append(matchOperation);
	...
	RollupResultDTO resultDTO =  executeSingleAggregation(aggregation.build(), RollupResultDTO.class);
	
	return resultDTO != null ? resultDTO : new RollupResultDTO(getDefaults(aggregationFields));
    }
    
    public RollupResultDTO averageCallFieldsByCompany(String companyId, DateTime startDate, DateTime endDate, List<CallField> fields) throws InvalidArgumentsException {
	Criteria matchCriteria = getCompanyConstrained(companyId).andOperator(getDateConstrained(startDate, endDate), hasBeenAnalyzed);
	
	return averageCallFields(matchCriteria, fields);
    }
    
    public RollupResultDTO averageCallFieldsByUser(String userId, DateTime startDate, DateTime endDate, List<CallField> fields) throws InvalidArgumentsException {
	Criteria matchCriteria = getUserConstrained(userId).andOperator(getDateConstrained(startDate, endDate), hasBeenAnalyzed);
	
	return averageCallFields(matchCriteria, fields);
    }
    
    public List<RollupResultDTO> rollupCallField(Criteria matchCriteria, List<CallField> fields) throws InvalidArgumentsException {
	return rollupCallField(matchCriteria, RollupCadence.DAILY, fields);
    }
    
    public List<RollupResultDTO> rollupCallField(Criteria matchCriteria, RollupCadence cadence, List<CallField> fields) throws InvalidArgumentsException {
	List<AggregationField> aggregationFields = getFields(fields, false);
	MatchOperation matchOperation = match(matchCriteria);
    ...
    public List<RollupResultDTO> rollupCallFieldByCompany(String companyId, DateTime startDate, DateTime endDate, RollupCadence cadence, List<CallField> fields) throws InvalidArgumentsException {
	Criteria matchCriteria = getCompanyConstrained(companyId).andOperator(getDateConstrained(startDate, endDate), hasBeenAnalyzed);
	
	return rollupCallField(matchCriteria, cadence, fields);
    }
    
    public List<RollupResultDTO> rollupCallFieldByUser(String userId, DateTime startDate, DateTime endDate, RollupCadence cadence, List<CallField> fields) throws InvalidArgumentsException {
	Criteria matchCriteria = getUserConstrained(userId).andOperator(getDateConstrained(startDate, endDate), hasBeenAnalyzed);
