...
public class CallbackController extends ApiController {
  ...
  public ResponseEntity<?> postCallback(
      @PathVariable String type, @PathVariable String platform, HttpServletRequest request)
      throws HandlerNotFoundException, CallbackException {
    String requestPath =
    ...
    CallbackResult<?> result =
        callbackService
            .getHandler(type, platform, request.getMethod())
            .handleRequest(requestPath, new CallbackBody(parameters, body.toString()));

    if (result == null) {
      return new ResponseEntity<>(
          "Unable to complete callback at [" + requestPath + "]", HttpStatus.BAD_REQUEST);
    }

    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(result.getContentType());

    return new ResponseEntity<>(result.getRenderedResult(), headers, result.getStatus());
  }

  @RequestMapping(value = CALLBACK + PARAM_TYPE + PARAM_PLATFORM, method = RequestMethod.GET)
