...
public class MapperUtils {
  ...
  public <T> PageDTO<T> mapPage(Page<?> page, List<T> content) {
    Pagination pagination =
        Pagination.builder().page(page.getNumber()).pageSize(page.getSize()).build();

    if (page.getSort() != null) {
      List<SortField> sortFields = new ArrayList<SortField>();
      Sort sort = page.getSort();
      asStream(sort.iterator())
          .forEach(
              x -> {
  ...
  public <T1, T2> PageDTO<T2> mapPage(Page<T1> page, Class<T2> clazz) {
    List<T2> content =
        page.getContent().stream().map(x -> modelMapper.map(x, clazz)).collect(Collectors.toList());
