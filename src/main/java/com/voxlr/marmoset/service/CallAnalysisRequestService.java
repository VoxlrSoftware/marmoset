...
public class CallAnalysisRequestService {
	...
	try {
		...
		CallAnalysisRequest request = CallAnalysisRequest.builder()
			...
			.transcriptionUrl(call.getTranscriptionUrl()).build();
	...
	try {
	    ...
	    HttpPost post = new HttpPost(combinePaths(appProperties.getAnalysisUrl(), "process_punctuation"));
		...
		throw new Exception("Received bad status from request [" + response.getStatusLine().toString() + "]");
	    }
	...
	try {
	    ...
	    CallAnalysisRequestDTO requestDTO = CallAnalysisRequestDTO.builder()
		    .callId(call.getId())
