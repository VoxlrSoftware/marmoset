...
public class CallbackService implements ApplicationContextAware, InitializingBean {
  ...
  public String getCallbackPath(CallbackType type, Platform platform) {
    return combinePaths(
        appProperties.getExternalApiUrl(), "callback", type.getName(), platform.getName());
  }

  private final HashMap<String, CallbackHandler<?>> callbackHandlers = new HashMap<>();
              ...
              try {
                if (CallbackHandler.class.isAssignableFrom(handlerClass)) {
                  ...
                  HashMap<String, Object> methodMap =
                      getAnnotationMembers(handlerClass, Callback.class);

                  addHandler(handler, methodMap);
                }
              } catch (Exception e) {
                throw new RuntimeException(
                    "Illegal use of @Callback for handler [" + handlerClass.getName() + "]", e);
              }
            });
  }
  ...
  public CallbackHandler<?> getHandler(String type, String platform, String method)
