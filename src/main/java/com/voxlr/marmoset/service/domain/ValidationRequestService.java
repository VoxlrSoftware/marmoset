...
public class ValidationRequestService {
  ...
  @Autowired private ValidationRequestRepository validationRequestRepository;
  ...
  public ValidatePhoneResponseDTO validatePhoneNumber(
      AuthUser authUser, Phoneable<?> entity, PhoneNumberHolder phoneNumber) {
    ...
    if (comparePhoneNumbers(entity.getPhoneNumber(), phoneNumber)) {
      ...
      try {
        com.twilio.rest.api.v2010.account.ValidationRequest twilioValidationRequest =
            twilioService.validatePhoneNumber(phoneNumber);
        ValidationRequest request =
            createRequest(authUser, entity, phoneNumber, twilioValidationRequest.getCallSid());
  ...
  public ValidationRequest createRequest(
      AuthUser authUser, Phoneable<?> entity, PhoneNumberHolder phoneNumber, String requestId) {
    ValidationRequest request =
    ...
    ValidationRequest currentRequest =
        validationRequestRepository.locateRequest(authUser.getId(), entity.getId());
    ...
    if (request != null) {
      ...
      if (response) {
        try {
          ...
          Phoneable<?> entity =
              (Phoneable<?>) mongoTemplate.findById(request.getEntityId(), entityType);
