...
public class CallAnalysisService {
  ...
  private void handleAnalysisResult(Call call, CallAnalysisResult result) {
    ...
    List<PhraseAnalysis> phraseAnalysis =
        expectedPhrases
            .stream()
            .map(
                phrase -> {
                  ...
                  try {
                    PhraseResult phraseResult =
                        result
                            .getPhrases()
                            .stream()
                            .filter(isEqual(PhraseResult::getPhrase, pAnalysis.getPhrase()))
                            .findFirst()
                            .get();
                    phraseResult
                        .getDetections()
  ...
  private void postProcessAnalysis(Analysis analysis) {
    ...
    int detectionCount =
        (int)
            asStream(analysis.getPhraseAnalysis())
                .map(PhraseAnalysis::getDetections)
                .filter(
                    detections ->
                        asStream(detections)
                            .anyMatch(
                                detection -> detection.wasDetected(DEFAULT_DETECTION_THRESHOLD)))
                .count();
    analysis.setDetectedPhraseCount(detectionCount);
    analysis.setDetectionRatio(
        phraseCount > 0 ? ((double) detectionCount) / ((double) phraseCount) : 0.0);
  }
}
